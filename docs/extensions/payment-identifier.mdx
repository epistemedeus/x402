...
3. Server caches responses keyed by payment ID and a fingerprint of the HTTP request (with configurable TTL)
4. Retry requests with the same payment ID and the same request fingerprint return cached responses without re-processing payment. A different method, path, query, or body with that ID returns `409 Conflict` and must not grant access.
...
const idempotencyCache = new Map<string, { timestamp: number; fingerprint: string; response: unknown }>();
const pendingFingerprints = new Map<string, string>();
const CACHE_TTL_MS = 60 * 60 * 1000; // 1 hour

function requestFingerprint(req: { method: string; originalUrl: string }, body: Buffer, payload: { accepted?: Record<string, string> }): string {
...
const resourceServer = new x402ResourceServer(facilitatorClient)
  ...
  .onAfterSettle(async ({ paymentPayload }) => {
    ...
    const fingerprint = paymentId ? pendingFingerprints.get(paymentId) : undefined;
...
`onProtectedRequest` can only return `{ grantAccess: true }` (runs the resource handler) or `{ abort: true }` (HTTP 403). It cannot return 409. Check the fingerprint in application middleware **before** payment middleware, and never call `grantAccess` on drift:

```typescript
app.use((req, res, next) => {
  if (!req.headers["x-payment"] && !req.headers["payment-signature"]) return next();
  try {
    ...
    const fingerprint = requestFingerprint(req, Buffer.alloc(0), paymentPayload);
...
- HTTP method and canonical resource path (sorted query, no fragment)
- Raw request body SHA-256
